...
const Paws = ({ isWaving }) => (
  <>
    <div 
      className={`absolute bottom-8 left-12 w-8 h-4 bg-gray-400 rounded-full transform origin-right transition-transform duration-300
...
const Tail = ({ isHappy }) => (
  <div 
    className={`absolute bottom-16 right-4 w-16 h-4 bg-gray-400 rounded-full transform -rotate-45 transition-transform duration-300
      ${isHappy ? 'animate-wag' : ''}`}
    style={{
      '@keyframes wag': {
